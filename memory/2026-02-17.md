...
## Flush pre-compacción (18:17 CST) — cierre bloque operativo final QTS

- Se completó el bloque opcional pendiente de readiness en `QTS-ARCHITECT` con implementación real en código y docs:
  - Alertas automáticas en live loop con persistencia en Redis:
    - `ALERTS:LAST`
    - `ALERTS:EVENT:<epoch_ms>`
  - Latency telemetry operativa:
    - `METRICS:LATENCY:TICK_TO_DECISION_MS`
    - `METRICS:LATENCY:DECISION_TO_FILL_MS`
    - `METRICS:LATENCY:TICK_TO_FILL_MS`
  - Integración en paths críticos: timeout submit, confirm fail, reconcile fail, strategy fail y emergency halt.

- Runbooks operativos añadidos:
  - `qts_core/docs/runbooks/emergency-halt-and-recovery.md`
  - `qts_core/docs/runbooks/live-trading-ops-checklist.md`

- Config/documentación actualizadas:
  - `conf/live.yaml` ahora incluye `alerts` + `telemetry.latency_keys`.
  - `qts_core/docs/redis_keys.md` actualizado con nuevas claves de alertas/latencia y aclaración de `METRICS:PNL_DAILY`.

- Validación técnica post-cambios:
  - `python3 -m py_compile` en módulos tocados: OK.
  - Suite relevante (`test_agents`, `test_live_flow`, `test_execution`, `test_backtest`): PASS.

- Commit del repositorio `QTS-ARCHITECT`:
  - `ebe8e7c` — `feat(ops): add alerting, latency telemetry, and live runbooks`.
